 ...
 *
 * @version $Id: EmailSendProbe.java,v 1.8.2.2 2005-10-27 15:26:05 pierre Exp $
...
public class EmailSendProbe implements Runnable {
    ...
    */
                    ...
                    if (anode==null) {
                        ...
                        if (ttime>=ntime) {
                            ...
                            try {
                                ...
                                log.error("emailsendprobe : performTask failed : ( " +e.getMessage() + " )"+anode);
                                tasks.removeElement(anode);
    ...
    public synchronized boolean putTask(MMObjectNode node) {
        if (node.getIntValue("mailtype") < 0) {
            node.setValue("mailtype",Email.TYPE_ONESHOT);
            node.commit();
        }
    ...
    public synchronized void checkQueue() {
        ...
        BasicLegacyConstraint constr = new BasicLegacyConstraint("mailstatus != 2 AND (mailtime<"+(ttime+maxtasktime)+" OR mailtime IS NULL)");
