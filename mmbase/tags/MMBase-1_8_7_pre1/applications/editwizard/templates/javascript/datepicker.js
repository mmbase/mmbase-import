...
//	email :	fuushikaden@yahoo.com
	...
	var imgDir = "../media/datepicker/"			// directory for images ... e.g. var imgDir="/img/"

	var	formObject, fieldName, crossobj, crossMonthObj, crossYearObj, monthSelected, yearSelected, dateSelected, omonthSelected, oyearSelected, odateSelected, monthConstructed, yearConstructed, intervalID1, intervalID2, timeoutID1, timeoutID2, ctlToPlaceValueDay, ctlToPlaceValueMonth, ctlToPlaceValueYear, ctlNow, dateFormat, nStartingYear
    ...
    */
	...
	if (dom)
	{
		...
		document.write ("</table></div><div id='selectMonth' onmouseover=\"\" style='z-index:+999;position:absolute;visibility:hidden;'></div><div id='selectYear' onmouseover=\"\" style='z-index:+999;position:absolute;visibility:hidden;'></div>");
	}

	if (startAt!=0)
	{
		var dayZero = dayName[0];
		for	(i=0; i<6;	i++) {
	...
	function closeCalendar() {
		...
		tempFieldName = fieldName + "_day";
    formObject[tempFieldName].selectedIndex = dateSelected - 1;
		tempFieldName = fieldName + "_month";
   ...
   function WeekNbr(n) {
	...
	function popUpCalendar(ctl, format, frameOffSetLeft, frameOffSetTop, object, name,event) {
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				if ( formatChar	!= "" )
				{
					...
					for	(i=0;i<3;i++)
					{
						...
						{
							tempFieldName = fieldName + "_day"
							dateSelected = parseInt(formObject[tempFieldName].value)
							tokensChanged ++
						}
						else if	((aFormat[i]=="m") || (aFormat[i]=="mm"))
						{
							tempFieldName = fieldName + "_month"
							monthSelected =	parseInt(formObject[tempFieldName].value) - 1
							tokensChanged ++
						}
						else if	(aFormat[i]=="yyyy")
						{
							tempFieldName = fieldName + "_year"
							yearSelected = parseInt(formObject[tempFieldName].value)
							tokensChanged ++
						}
			...
			{
				hideCalendar()
				if (ctlNow!=ctl) {popUpCalendar(ctl, format, leftpos, toppos, formObject, fieldName)}
			}
			ctlNow = ctl
		}
	}
