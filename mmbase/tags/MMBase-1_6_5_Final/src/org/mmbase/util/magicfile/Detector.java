...
public class Detector {
    private static Logger log = Logging.getLoggerInstance(Detector.class.getName());
	...
	if (hit) {
	    ...
	    if (m > 0) {
		for (int i=0;i<m;i++) {
		    child = (Detector)childList.get(i);
	...
	if (! valid) {
		...
		for (int i=0;i<childList.size();i++) {
		    res.append("> ").append(((Detector)childList.get(i)).toString());
		}
	    }
	    return res.toString();
	}
