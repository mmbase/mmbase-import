...
public class Generate {
    ...
    public static boolean generateThreads(int generatecount, int generatedelay, String inforum, String inpostarea) {
        log.info("GENERATE THREADS : count=" + generatecount + " delay=" + generatedelay + " in forum=" + inforum + " postarea="
                + inpostarea);
        if (state == 0) {
            state = 1;
    ...
    public static boolean generateReplys(int generatecount, int generatedelay, String inforum, String inpostarea) {
        log.info("GENERATE REPLYS : count=" + generatecount + " delay=" + generatedelay + " in forum=" + inforum + " postarea="
                + inpostarea);
        if (state == 0) {
            state = 1;
