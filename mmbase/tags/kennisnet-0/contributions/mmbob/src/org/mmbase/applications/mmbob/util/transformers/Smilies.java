 ...
 * @version $Id: Smilies.java,v 1.4.2.3 2007-01-22 09:30:40 ernst Exp $
 * @since MMBob-1.0
 */
public class Smilies extends StringTransformer implements CharTransformer {
    ...
    protected void initPatterns(String themeID, String smileySetID, String smileyKey) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("There are " + smileySet.getCount() + " smilies in this set (theme: " + themeID + "smileySetID: " + smileySetID
                    + "smileyKey: " + smileyKey + ")");
        }
        int i = 0;
    ...
    /**
     * Default transform method (with no other params). It will transform the originalString into a version with the
     * graphical version of the smilies
     * 
    ...
    public String transform(String originalString, String themeID, String imagecontext) {
        ...
        for (int i = 0; i < matchers.length; i++) {
            ...
            while (matchers[i].find()) {
                ...
                matchers[i].appendReplacement(resultBuffer, "<img src=\"" + imagecontext + "/" + themeID + "/" + smileySetID + "/"
                        + (String) smileySet.getImage(matchers[i].group()) + "\" />");
            }
    ...
    /*
     * this methods aren't used at the moment, and maybe they must be removed, but I'm not sure (GvE) protected static
     * void init() { smilies = new HashMap(); //smilies.put(":)","images/smile.gif"); String themeid="MMBaseWebsite";
     * Theme theme=ThemeManager.getTheme(themeid); if (theme!=null) { is=theme.getImageSet("blue"); Iterator
     * i=is.getImageIds(); while (i.hasNext()) { String id=(String)i.next();
     * smilies.put(id,"/thememanager/images/"+themeid+"/blue/"+is.getImage(id)); }
     * 
     *  } else { log.error("Can't find theme for smilies"); } }
     * 
     * public static void init(String themeID, String smileysetID) { log.debug("going to get theme with id = " +
     * themeID); String assignedID = ThemeManager.getAssign(themeID); Theme theme = ThemeManager.getTheme(assignedID);
     * if (theme != null) { Map imageSets = theme.getImageSets("smilies"); Iterator i = imageSets.entrySet().iterator();
     * while(i.hasNext()) { ImageSet is = (ImageSet)imageSets.get(((Map.Entry)i.next()).getKey());
     * smileySets.put(themeID,is); }
     * 
     * //is=theme.getImageSet("blue"); //Iterator i=is.getImageIds(); // while (i.hasNext()) { //String
     * id=(String)i.next(); //smilies.put(id,"/thememanager/images/"+themeid+"/blue/"+is.getImage(id)); //} } else {
