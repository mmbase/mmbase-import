 ...
 * @author Nico Klasens
 * @version $Id: ConvertImageMagick.java,v 1.42.2.1 2003-01-24 12:23:47 nico Exp $
 */
public class ConvertImageMagick implements ImageConvertInterface {
	...
	public void init(Map params) {
		...
		*/
		...
		try {
			...
			log.error(
				"Was not permitted to execute (because of SecurityManager) "
					+ converterPath
					+ " ("
					+ e.toString()
					+ ") conv.root='"
			...
			log.error(
				"An I/O error occured while executing "
					+ converterPath
					+ " ("
					+ e.toString()
					+ ") conv.root='"
	...
	private byte[] convertImage(
		...
		if (log.isDebugEnabled()) {
			log.debug(
				"command:" + command + " in " + new File("").getAbsolutePath());
		}
