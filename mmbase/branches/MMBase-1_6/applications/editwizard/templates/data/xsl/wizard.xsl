...
>
  ...
  @version $Id: wizard.xsl,v 1.80.2.4 2003-03-28 18:20:52 michiel Exp $
  -->
        ...
        <xsl:when test="(@ftype='datetime') or (@ftype='date') or (@ftype='time')">
          <div>
            ...
            <xsl:if test="(@ftype='datetime') or (@ftype='date')">
              ...
              </select><xsl:value-of select="$time_daymonth" /><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
            ...
            <xsl:if test="@ftype='datetime'">
              <span class="time_at"><xsl:value-of select="$time_at" /></span>
            </xsl:if>
