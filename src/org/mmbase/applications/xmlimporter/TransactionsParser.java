...
public class TransactionsParser extends DefaultHandler {
   ...
   private final static String ATTRIBUTE_DISPOSE
   = "disposeWhenNotReferenced";
   ...
   TransactionsParser.class.getName());
   ...
   Attributes attributes) throws SAXException {
      ...
      try {
         if (name.equals(ELEMENT_TRANSACTIONS)) { // transactions
            // Get attributes.
            String attrExceptionPage
            = attributes.getValue(ATTRIBUTE_EXCEPTION_PAGE);
            String attrReportFile
            = attributes.getValue(ATTRIBUTE_REPORT_FILE);
            ...
            try {
               ...
               "Failed to create reportFile "+reportFile+": " + e);
            }
            ...
            transaction = Transaction.createTransaction(uti, id, commit,
            ...
            = Boolean.valueOf(attributes.getValue(ATTRIBUTE_DISPOSE))
            .booleanValue();
            ...
            String id = attributes.getValue(ATTRIBUTE_ID); // id
            try {
               int mmbaseId = Integer.parseInt(
               attributes.getValue(ATTRIBUTE_MMBASE_ID)); // mmbaseId
               
               // Access object.
               tmpObject = transaction.accessObject(id, mmbaseId);
            } catch (NumberFormatException e) {
               throw new SAXException("invalid attribute mmbasid=\""
               + attributes.getValue(ATTRIBUTE_MMBASE_ID) + "\"");
            }
         ...
         name.equals(ELEMENT_MARK_OBJECT_DELETE)) { // markObjectDelete
            // Get attributes.
            try {
               int mmbaseId = Integer.parseInt(
               attributes.getValue(ATTRIBUTE_MMBASE_ID)); // mmbaseId
               boolean deleteRelations = Boolean.valueOf( // deleteRelations
               attributes.getValue(ATTRIBUTE_DELETE_RELATIONS))
               .booleanValue();
            ...
            "transaction operator \""+ name + "\" doesn't exist");
         }
      ...
      try {
         if (name.equals(ELEMENT_TRANSACTIONS)) { // transactions
            ...
            try {
               ...
               throw new SAXException(
               "Failed to write to reportFile " +reportFile+ ": " + e);
            }
         ...
         name.equals(ELEMENT_MARK_OBJECT_DELETE)) { // markObjectDelete
            ...
            try {
               objectFinder = (SimilarObjectFinder)Class.forName(className)
               .newInstance();
            ...
            try {
               objectMerger = (ObjectMerger) Class.forName(className)
               .newInstance();
   ...
   throws org.xml.sax.SAXException {
      ...
      if (fieldName != null) {
         ...
         if (url != null) {
            throw new SAXException(
            "Can not set field value from both url and element value.");
         }
         if (fieldValue == null) {
   ...
   throws IOException, TransactionHandlerException {
      ...
      try {
         // Create validating SAX perser.
         parser = new SAXParser();
         parser.setFeature("http://xml.org/sax/features/validation", true);
      } catch (SAXException e1) {
         throw new TransactionHandlerException(
         "Failed to create validating SAXParser: \n"
         + "org.apache.xerces.parsers.SAXParser." + " hallo " + e1);
      }
      ...
      try {
         ...
         if (transaction != null) {
            if (log.isDebugEnabled()) {
               log.debug("About to roll back transaction "
               + transaction.getKey() + " because an error occurred.");
            }
            try {
               transaction.delete();
            } catch (Exception e4) {
               log.debug("Failed to roll back transaction "
               + transaction.getKey() + " after an error occurred.");
            }
         }
   ...
   private void appendReportFile(String text) throws IOException {
      ...
      if (reportFile != null) {
         try {
            ...
            new FileOutputStream(reportFile.getPath(), true), ENCODING));
            out.write(text);
         } finally {
            if (out != null) {
               out.close();
