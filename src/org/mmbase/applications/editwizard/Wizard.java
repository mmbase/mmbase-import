 ...
 * @since MMBase-1.6
 * @version $Id: Wizard.java,v 1.74.2.19 2003-10-15 10:29:36 nico Exp $
 *
 */
public class Wizard implements org.mmbase.util.SizeMeasurable {
   ...
   */
   ...
   public Document createPreHtml(Node wizardSchema, String formid, Node data,
      ...
      NodeList globals = Utils.selectNodeList(wizardSchema,
      ...
      Node step = Utils.selectSingleNode(wizardSchema,
            "./steps/step[@form-schema='" + formid + "']");

      if (step != null) {
         // Yes. we have step information. Let's add info about that.
         String otherformid = "";
         Node prevstep = Utils.selectSingleNode(step,
               "./preceding-sibling::step[1]");
         ...
         Node nextstep = Utils.selectSingleNode(step,
               "./following-sibling::step[1]");
      ...
      for (int f = 0; f < formlist.getLength(); f++) {
         ...
         NodeList props = Utils.selectNodeList(form,
               "title|subtitle|description");
      ...
      NodeList optionlists = Utils.selectNodeList(wizardnode,
            ".//optionlist[@select]");
      ...
      for (int i = 0; i < optionlists.getLength(); i++) {
         ...
         Node list = Utils.selectSingleNode(wizardSchema,
               "/*/lists/optionlist[@name='" + listname + "']");
         ...
         if ((query != null) && ((currentTime - lastExecuted) > queryTimeOut)) {
            ...
            try {
               ...
               log.debug("Error during query, proceeding with next list: " +
                  e.toString());

               Element option = list.getOwnerDocument().createElement("option");
               option.setAttribute("id", "-");
               Utils.storeText(option,
                  "Error: query for '" + listname + "' failed");
               optionlist.appendChild(option);
         ...
         for (int i = 0; i < externalReferences.getLength(); i++) {
            ...
            try {
               ...
               if (externalDocument == null) {
                  throw new WizardException(
                     "Could not load and parse included file. Filename:" +
                     file);
               }
               ...
               if (hash == -1) {
                  ...
                  String xpath = externalId.substring(9, externalId.length() -
                        1);
               ...
               externalPart = parent.insertBefore(targetdoc.importNode(
                        externalPart, true), referer);
      ...
      if (nodeName.equals("field")) {
         ...
         if (fdatapath != null) {
            if (searchString != null) {
               throw new WizardException(
                  "It does not make sense to specify 'fdatapath' _and_ 'searchdir' attributes");
            }

            if (role != null) {
               throw new WizardException(
                  "It does not make sense to specify 'fdatapath' _and_ 'role' attributes");
            }
   ...
   private Node addListItem(String listId, String dataId, String destinationId,
      ...
      Node listnode = Utils.selectSingleNode(schema,
            ".//list[@fid='" + listId + "']");
      Node objectdef = null;
      ...
      if (log.isDebugEnabled()) {
         log.debug("Creating object " + objectdef.getNodeName() + " type " +
            Utils.getAttribute(objectdef, "type"));
      }
   ...
   public void mergeConstraints(Node fieldDef, Node fieldNode) {
      ...
      if ((objectType == null) || (fieldName == null)) {
         log.debug(
            "wizard.mergeConstraints: objecttype or fieldname could not be retrieved for this field. Field:");
   ...
   public Node getConstraints(String objecttype, String fieldname) {
      ...
      if (con == null) {
         ...
         if (con == null) {
            // something is wrong.
            log.debug(
               "wizard.getConstraints: Could not retrieve MMBase constraints for objecttype:" +
               objecttype);
      ...
      Node fieldcon = Utils.selectSingleNode(con,
      ...
      public int compare(Object o1, Object o2) {
         ...
         if (compareByNumber) {
            try {
               ...
               log.error("Invalid field values (" + order1 + "/" + order2 +
                  "):" + e);
